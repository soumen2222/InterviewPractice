...
public class A_E3_0510_TH_VEN extends PULLBaseTestCase {

	public static void main(String[] args) {
		try {
			A_E3_0510_TH_VEN testClass = new A_E3_0510_TH_VEN();
	...
	public void test() throws Exception {
		...
		uiUserPrompt.Prompt(resourceFileReader.Prereq_NoEvents_Message()
				+ "\n\n" + resourceFileReader.TestCase_0510_UIUserPromptText(),
				2);
		...
		// Load OadrDistributeEventResponse 
		...
		if (oadrDistributeEvent.getOadrEvent().size() != 4) {
			LogHelper.setResult(LogResult.FAIL);
			LogHelper
		...
		if(!DistributeEventSignalHelper.isExpectedReceived("A",ResponseRequiredType.ALWAYS,0, oadrDistributeEvent.getOadrEvent().get(1))){
		...
		if(!DistributeEventSignalHelper.isExpectedReceived("P",ResponseRequiredType.ALWAYS,0, oadrDistributeEvent.getOadrEvent().get(2))){
		...
		if(!DistributeEventSignalHelper.isExpectedReceived("P",ResponseRequiredType.ALWAYS,0, oadrDistributeEvent.getOadrEvent().get(3))){
