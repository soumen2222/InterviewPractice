...
public class A_E3_0490_TH_VEN extends PULLBaseTestCase {

	public static void main(String[] args) {
		try {
			A_E3_0490_TH_VEN testClass = new A_E3_0490_TH_VEN();
	...
	public void test() throws Exception {
		...
		uiUserPrompt.Prompt(resourceFileReader.Prereq_NoEvents_Message()
				+ "\n\n" + resourceFileReader.TestCase_0490_UIUserPromptText(),
				1);
		...
		String message = resourceFileReader.TestCase_0490_SecondUIUserPrompt();
		...
		if(!DistributeEventSignalHelper.isExpectedReceived("A",ResponseRequiredType.ALWAYS,1, oadrDistributeEvent2.getOadrEvent().get(0))){
			LogHelper.setResult(LogResult.FAIL);
			LogHelper.addTrace("Test Case has Failed");
			return;
		}
		...
		if (responseCode2 != null
			...
			LogHelper.setResult(LogResult.FAIL);
			LogHelper
					.addTrace("response code not 200");
			LogHelper.addTrace("Test Case has Failed");
		}
