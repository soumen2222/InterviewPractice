...
public class A_E3_0720_TH_VEN extends PULLBaseTestCase {

	public static void main(String[] args) {
		try {
			A_E3_0720_TH_VEN testClass = new A_E3_0720_TH_VEN();
	...
	public void test() throws Exception {
		...
		uiUserPrompt.Prompt(resourceFileReader.Prereq_NoEvents_Message()
				+ "\n\n" + resourceFileReader.TestCase_0720_UIUserPromptText(),
				1);
		...
		// Object so that you can get read values, in this case we need to find
		...
		if (oadrDistributeEvent.getOadrEvent().size() != 1) {
			LogHelper.setResult(LogResult.FAIL);
			LogHelper
					.addTrace("Expected OadrEvent was not received in OadrDistributeEvent");
			LogHelper.addTrace("Test Case has Failed");
			return;
		}
