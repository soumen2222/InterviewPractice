...
public class A_E3_0420_TH_VEN extends PULLBaseTestCase {

	public static void main(String[] args) {
		try {
			A_E3_0420_TH_VEN testClass = new A_E3_0420_TH_VEN();
	...
	public void test() throws Exception {
		...
		uiUserPrompt.Prompt(resourceFileReader.Prereq_NoEvents_Message()
				+ "\n\n" + resourceFileReader.TestCase_0420_UIUserPromptText(),
				1);
		...
		OadrRequestEventType oadrRequestEvent = RequestEventSignalHelper.loadOadrRequestEvent("oadrRequestEvent_Default.xml");
		oadrRequestEvent.getEiRequestEvent().setVenID(propertiesFileReader.getVenID());
		String strOadrRequestEvent = SchemaHelper.getRequestEventAsString(oadrRequestEvent);
		String strOadrDistributeEventResponse = VenToVtnClient.post(strOadrRequestEvent);
		
		OadrDistributeEventType oadrDistributeEvent = DistributeEventSignalHelper.createOadrDistributeEventFromString(strOadrDistributeEventResponse);
		...
		// Convert the oadrResponse String received to OadrResponse.
		...
		// Test Case condition
		if (responseCode.equals("200")&&DistributeEventSignalHelper.isExpectedReceived("A",ResponseRequiredType.ALWAYS,0, oadrDistributeEvent.getOadrEvent().get(0))) {
