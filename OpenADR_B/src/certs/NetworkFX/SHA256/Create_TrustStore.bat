...
rem
keytool -import -keystore %TRUSTSTORE% -storepass %PASSWORD% -alias RSA_SPCA0001 -file TEST_OpenADR_RSA_SPCA0002_Cert_PEM.crt

rem Import intermediate CA certificate for VEN device certificates to truststore
rem
keytool -import -keystore %TRUSTSTORE% -storepass %PASSWORD% -alias RSA_MCA0001  -file TEST_OpenADR_RSA_MCA0002_Cert_PEM.crt
