...
@Stateless
public class AccountCreaterClientQueueBean implements AccountCreaterClientQueue.L,AccountCreaterClientQueue.R {
    ...
    @Resource(mappedName = "java:/queue/accountCreationQueue")
    private Queue AccountCreationQueue;

    private Logger log = Logger.getLogger(AccountCreaterClientQueueBean.class);
	...
	public void sendMessage(ProgramPartyDTO message) {
        ...
        Session session = null;
         try {
            ...
            final MessageProducer publisher = session.createProducer(AccountCreationQueue);
	...
	private final javax.jms.Message createJmsMessage(Session session, ProgramPartyDTO message) throws JMSException
	{
        final ObjectMessage jmsMsg = session.createObjectMessage(message);
        return jmsMsg;
	}
  
	protected long timeToLive(ProgramPartyDTO message) {
